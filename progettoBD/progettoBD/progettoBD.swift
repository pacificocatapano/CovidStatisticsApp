...
//  progettoBD
//
//  Created by Salvatore Giugliano on 11/05/2020.
//  Copyright © 2020 Pacifico Catapano. All rights reserved.
//

import Foundation

public class progettoBD {
                    ...
                    do {
                        ...
                        if let array = jsonResult as? Array<Dictionary<String, Any>> {
                            ...
                            if !array.isEmpty {
                                ...
                                completion(Location(lat: (array[0]["lat"] as! NSString).floatValue, lon: (array[0]["lon"] as! NSString).floatValue, countryCode: countryCode, country: country, state: state, county: county, postcode: postcode, city: city, cityDistrict: cityDistrict, road: road, houseNumber: houseNumber))
        ...
        session.dataTask(with: queryURL, completionHandler: { data, response, error -> Void in
            ...
            do {
                ...
                if let dict = jsonResult as? Dictionary<String, Any> {
                    ...
                    completion(nil, Location(lat: (dict["lat"] as! NSString).floatValue, lon: (dict["lon"] as! NSString).floatValue, countryCode: countryCode, country: country, state: state, county: county, postcode: postcode, city: city, cityDistrict: cityDistrict, road: road, houseNumber: houseNumber))
                }
...
public class Location {
    ...
    required public init(lat: Float, lon: Float, countryCode: String?,  country: String?, state: String?, county: String?, postcode: String?, city: String?, cityDistrict: String?, road: String?, houseNumber: String?) {
        self.latitude = lat
        self.longitude = lon
